...
func TestOptimisedProductWithoutSelf(t *testing.T) {
	type args struct {
		arr []int
	}
	tests := []struct {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got, err := OptimisedProductWithoutSelf(tt.args.arr)
			if (err != nil) != tt.wantErr {
				t.Errorf("OptimisedProductWithoutSelf() error = %v, wantErr %v", err, tt.wantErr)
				return
			}
			if !reflect.DeepEqual(got, tt.want) {
				t.Errorf("OptimisedProductWithoutSelf() got = %v, want %v", got, tt.want)
			}
		})
	}
}
